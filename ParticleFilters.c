 ...
 ***************************************************************/
  ...
  list = NULL;
    ...
    // allocate space for each particle
		particle *p = (struct particle *) malloc(sizeof(struct particle));

    // place the particle in an empty space
    do {
      p->x = rand() % sx;
      p->y = rand() % sy;
 ...
 ****************************************************************/
    ...
    // tee is the factor with which the probabilities increse or decrease.
    // treshold is the cut off point of 'good' particles.
    double new_prob = p->prob, tee = 1.3, treshold = 0.017603;

    int i;

    // for each particle update the likely hood as follows:
    // add noise to the particle and then compare that to the 
    // robot using gaussian with sigma 20 for the error spread.
    // If the particle is within a 'good' range from the robot
    // increase the probability, else derease the probability.
    for (i = 0; i < 16; i++)
      new_prob += (GaussEval(GaussianNoise(p->measureD[i], noise_sigma) 
                              - rob->measureD[i], 20) >= treshold)
                                    ? new_prob * tee : new_prob / tee;
 ...
 */
  ...
  double dist = 2;
  particle *cur_par, *par_copy, *new_par;
  double total, cut_off = 0.0000001;
  int i, j, count, n_par, r, finder;
   ...
   ******************************************************************/
    ...
    while (cur_par != NULL) {
      ...
      // move every robot a certain distance dist     
      move(cur_par, dist);

      // if the particle hit the wall, turn around and head in a random
      // direction
      if (hit(cur_par, map, sx, sy)) {
	     cur_par->theta += 180 % 360;
	     move(cur_par, dist);
	     cur_par->theta = rand() % 360;
      }

      // scan the true surrounding of the particle 
      ground_truth(cur_par, map, sx, sy);
   ...
   *******************************************************************/
    ...
    // compute the likely hood of each particle and update the 
    // probability
    while (cur_par != NULL) {

     computeLikelihood(cur_par, robot, 10);
     total += cur_par->prob;
    ...
    // normalize the list of particles
    while (cur_par != NULL) {

     cur_par->prob = cur_par->prob / total;
   ...
   *******************************************************************/
    ...
    // copy over all the particles that have probability greater than 
    // the cut off cut_off
    for (i = 0; i < n_particles; i++) {

      // if the particle has a high enough probability copy it over to the 
      // new list
      if (cur_par->prob > cut_off) {
    ...
    // expand the new list to get 'n_particles' particles
    while (n_par < n_particles) {
